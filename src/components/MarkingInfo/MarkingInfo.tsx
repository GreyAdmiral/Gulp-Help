      ...
      <section className={styles.markingInfo}>
         ...
         <article className={classNames(styles.markingInfoItem, styles.markingInfoStruct)}>
            ...
            <p>
               ...
               <code>components</code>, <code>modules</code> и <code>plugins</code>. В первую сложены «компоненты»
               разметки, во вторую — вспомогательные «компоненты» для первой, в третью — разметка от используемых
               самописных (включая честно прихватизированные у «коллег по цеху») плагинов. Внутри первой папки можно
               найти папки <code>blocks</code>, куда складываются файлы с блоками для страницы, <code>elements</code>,
               где лежат мелкие элементы типа бургера или кастомного чекбокса, и <code>layout</code>, где лежат шаблоны
               для подключения информационного блока страницы (тег <code>head</code>), блока с подключением фавиконок и
               шаблон подключения скриптов. Внутри второй папки сложены мелкие компоненты для циклов (папки{' '}
               <code>elements</code>) и пропсы для некоторых импортируемых компонентов (папка <code>settings</code>).
               Туда же (в папку <code>contents</code>) можно складывать обертки для контентного текста (или не текста).
               Кроме того туда всегда можно добавить любые папки и файлы на собственный вкус и для собственных нужд.
            </p>
         </article>
      </section>
   );
